...
${c.rules ? 'RULES: ' + String(c.rules).slice(0, 1200) : ''}`
    ...
    const response = await client.messages.create({
      ...
      max_tokens: 1500,
      system: `You help a tournament organizer turn real attendee questions into a concise FAQ. Cluster the questions into the most common/important themes. For each, write a short, accurate answer (1-2 sentences) using the event info — if the info isn't available, write a helpful placeholder answer the organizer can fill in. Output ONLY a JSON array of 4-8 objects: [{"question":"...","answer":"..."}]. No prose, no markdown fences.`,
