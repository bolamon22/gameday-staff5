...
const ROLE_COLORS: Record<string, string> = {
  ref1: 'bg-indigo-100 text-indigo-700', ref2: 'bg-violet-100 text-violet-700',
      ...
      <div className="max-w-5xl mx-auto px-4 pt-6">
        ...
        <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setView('staff')}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${view === 'staff' ? 'bg-teal-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
              By staff
            </button>
          </div>
        </div>
          ...
          {dates.map(d => {
            ...
            return (
              <button key={d} onClick={() => setSelDate(d === selDate ? '' : d)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${selDate === d ? 'bg-teal-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                {label}
              </button>
            )
          })}
        ...
        {view === 'game' && (
          <div className="space-y-2">
            {filteredGames.length === 0 && <div className="text-center py-12 text-slate-400">No games found.</div>}
                  ...
                  <div className="px-5 py-2.5 flex flex-wrap gap-2">
                    {game.assignments.length === 0
                      ...
                      : game.assignments.map(a => (
                        <div key={a.id} className={`flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg ${ROLE_COLORS[a.role] || 'bg-slate-100 text-slate-600'}`}>
                          ...
                          <button onClick={() => removeAssignment(a.id)} disabled={removing === a.id}
                            className="ml-1 opacity-50 hover:opacity-100 transition-opacity text-current"><X size={12} /></button>
                        </div>
                      ))
                    }
                  </div>
        ...
        {view === 'staff' && (
          <div className="space-y-3">
            {staffList.length === 0 && <div className="text-center py-12 text-slate-400">No assignments in this view.</div>}
              ...
              return (
                <div key={worker.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                  <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-50 bg-slate-50">
                    ...
                    <span className="font-semibold text-slate-800 flex-1">{worker.name}</span>
                    <span className="text-xs text-slate-400">{wGames.length} game{wGames.length !== 1 ? 's' : ''}</span>
